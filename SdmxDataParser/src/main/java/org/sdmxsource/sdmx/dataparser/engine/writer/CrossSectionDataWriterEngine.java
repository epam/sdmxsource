...
/**
 * The type Cross section data writer engine.
 */
public class CrossSectionDataWriterEngine extends SdmxDataWriterEngine {
    ...
    private Logger LOG = LoggerFactory.getLogger(CrossSectionDataWriterEngine.class);
    ...
    private Map<String, String> componentVals = new HashMap<String, String>(); //A map containing the current component values for the current series/obs - i.e Time=2001, FREQ=A
    private String obsValue;
    private String timeConcept;

    /**
     * Instantiates a new Cross section data writer engine.
     *
...
//		}
    ...
    private boolean startNewCrossSectionalGroupIfRequired() throws XMLStreamException {
        if (currentPosition != POSITION.OBSERVATION) {
            ...
            if (!currentGroupKey.equals(newGroupKey)) {
                seriesWriter.writeEndElement();  //END Section
                seriesWriter.writeEndElement();  //END Group
                currentGroupKey = startNewCrossSection("Group", groupComponents);
                currentSectionKey = startNewCrossSection("Section", sectionComponents);
                return true;
            }
    ...
    private String startNewCrossSection(String sectionName, List<String> crossSectionComponentIds) throws XMLStreamException {
        ...
        startElement(seriesWriter, CROSS_NS, sectionName);  //WRITE THE START GROUP
        for (String currentComponent : crossSectionComponentIds) {
            if (componentVals.containsKey(currentComponent)) {
                seriesWriter.writeAttribute(currentComponent, componentVals.get(currentComponent));
            }
        }
    ...
    private void flushObservation() {
        ...
        try {
            ...
            startElement(seriesWriter, CROSS_NS, PrimaryMeasureBean.FIXED_ID);  //WRITE THE START GROUP
