...
/**
 * Assembles the Nameable parts of a bean to facilitate Builder&lt;K,V&gt; for the top level bean.
...
public class NameableBeanAssembler extends IdentifiableBeanAssembler {
     ...
     */
    public void assembleNameable(NameableType assembleInto, NameableBean assembleFrom) throws SdmxException {
        assembleIdentifiable(assembleInto, assembleFrom);
        if (validCollection(assembleFrom.getNames())) {
            assembleInto.setNameArray(getTextType(assembleFrom.getNames()));
        }
        if (validCollection(assembleFrom.getDescriptions())) {
            assembleInto.setDescriptionArray(getTextType(assembleFrom.getDescriptions()));
        }
    }
}
