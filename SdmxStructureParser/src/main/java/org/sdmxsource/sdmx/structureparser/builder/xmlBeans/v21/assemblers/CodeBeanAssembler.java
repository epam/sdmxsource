...
/**
 * Assembles a CodeBean to facilitate Builder&lt;K,V&gt; for the top level bean.
 * v2.1 SDMX uses inheritance, so thus can we, unlike in previous versions.
 * Builder&lt;K,V&gt; doesn't lend itself to inheritance.
 */
public class CodeBeanAssembler extends NameableBeanAssembler implements Assembler<CodeType, CodeBean> {

    @Override
    public void assemble(CodeType assembleInto, CodeBean assembleFrom) throws SdmxException {
        // Populate it from inherited super
        assembleNameable(assembleInto, assembleFrom);
        // Populate it using this class's specifics
        if (ObjectUtil.validString(assembleFrom.getParentCode())) {
            LocalItemReferenceType parent = assembleInto.addNewParent();
            RefBaseType ref = parent.addNewRef();
            ref.setId(assembleFrom.getParentCode());
        }
    }
}
