...
/**
 * The type Attribute super bean builder.
 */
public class AttributeSuperBeanBuilder extends ComponentSuperBeanBuilder<AttributeSuperBean, AttributeBean> {

    @Override
    public AttributeSuperBean build(AttributeBean buildFrom,
                                    SdmxBeanRetrievalManager retrievalManager,
                                    SuperBeans existingBeans) {
        CodelistSuperBean codelistBean = super.getCodelist(buildFrom, retrievalManager, existingBeans);
        ConceptSuperBean conceptSuperBean = super.getConcept(buildFrom, retrievalManager, existingBeans);
        return new AttributeSuperBeanImpl(buildFrom, codelistBean, conceptSuperBean);
    }
}
