...
/**
 * The type Sdmx beans v 2 reg doc builder.
 */
public class SdmxBeansV2RegDocBuilder extends AbstractSdmxBeansV2Builder implements Builder<SdmxBeans, RegistryInterfaceDocument> {
    ...
    public SdmxBeans build(RegistryInterfaceDocument rid) throws SdmxException {
        ...
        SdmxBeans beans = new SdmxBeansImpl(new HeaderBeanImpl(rit.getHeader()));
        if (rit.getSubmitStructureRequest() != null) {
            if (rit.getSubmitStructureRequest().getStructure() != null) {
                build(rit.getSubmitStructureRequest().getStructure(), beans);
            }
        }
        if (rit.getQueryStructureResponse() != null) {
            build(rit.getQueryStructureResponse(), beans);
        }
        return beans;
    }
    ...
    /**
     * Build beans from a Version 2.0 Query Response
     *
     * @param structures to build from
     * @return beans to build
     */
    private SdmxBeans build(org.sdmx.resources.sdmxml.schemas.v20.registry.QueryStructureResponseType structures, SdmxBeans beans) {
        Set<String> urns = new HashSet<String>();
            ...
            for (KeyFamilyType currentType : structures.getKeyFamilies().getKeyFamilyList()) {
                try {
                    if (isXsDataStructure(currentType)) {
                        ...
                        addIfNotDuplicateURN(beans, urns, mutable.getImmutableInstance());
                    } else {
                        addIfNotDuplicateURN(beans, urns, new DataStructureBeanImpl(currentType));
                    }
                } catch (Throwable th) {
        ...
        //ORGANISATION SCHEMES
        if (structures.getOrganisationSchemes() != null) {
            for (OrganisationSchemeType currentType : structures.getOrganisationSchemes().getOrganisationSchemeList()) {
                if (ObjectUtil.validCollection(currentType.getAgenciesList())) {
                    try {
                ...
                if (ObjectUtil.validCollection(currentType.getDataConsumersList())) {
                    try {
                ...
                if (ObjectUtil.validCollection(currentType.getDataProvidersList())) {
                    try {
