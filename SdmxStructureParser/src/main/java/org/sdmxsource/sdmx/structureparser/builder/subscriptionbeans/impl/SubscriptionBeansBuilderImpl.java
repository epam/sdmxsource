...
public class SubscriptionBeansBuilderImpl implements SubscriptionBeansBuilder {
    ...
    @Override
    public List<SubscriptionBean> build(SubmitSubscriptionsRequestType buildFrom) throws SdmxException {
        List<SubscriptionBean> returnList = new ArrayList<SubscriptionBean>();

        for (SubscriptionRequestType subscription : buildFrom.getSubscriptionRequestList()) {
            returnList.add(new SubscriptionBeanImpl(subscription.getSubscription()));
        }
        return returnList;
    }
}
